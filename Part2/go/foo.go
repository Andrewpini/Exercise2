...
func number_server(add_number <-chan int, control <-chan int, number chan<- int, finished chan<- bool) {
	var i = 0
	...
	for {
		select {
			...
			case j := <- add_number:
				i = i + j
			case incoming_ctrl_signal:= <-control:
				if incoming_ctrl_signal == GetNumber {
					number <- i
				} else if incoming_ctrl_signal == Exit {
...
func main() {
	...
	go number_server(add_number, control, number, finished)
